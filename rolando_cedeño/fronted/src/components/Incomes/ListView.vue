        ...
        <div class="row">
            <div class="col-xl-12 col-md-12">
                ...
                <div class="box col-md-12" id="head_income">
                    ...
                    <h4 class="header-title mb-3">LIST INCOMES</h4>                  
                                  
                    <div id="table_incomes_head_secc " >
                        <div class="table-responsive">                        
                            <table id="idtable_incomes_head" ref="table_incomes_head" class="stripe " style="width:100%">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <tr v-for="item in incomes" :key="item.idhead_income" id="body">
                                        <td scope="row" style="width:10%">{{item.idhead_income}}</td>
                ...
                <div class="box col-md-12" id="detail_income" style="display:none">  
                    ...
                    <div class="col-md-12" style="margin-top: 20px; margin-bottom: 30px;">
                        ...
                        <div class="col-md-6">
                            <ul class="nav nav-pills nav-stacked">
                                <li style="border-color: white"><a><i class="fa fa-list text-red"></i> <b class="text-black" style="font-weight: 600 !important">Id</b>: <span  id="idHeadIncome" style="font-weight: 500"></span></a></li>
                    ...
                    <div class="table-responsive" style="margin-top: 20px;">                        
                        <table id="idtable_det" ref="table_det" class="stripe " style="width:100%">
                            ...
                            <tbody>
                                <tr v-for="item in detail_incomes" :key="item.iddetail_income " id="body">
...
<script>
    ...
    export default{
        ...
        mounted(){
            this.geIncomes() //List of incomes registered in the database
            this.changeTitle() //Change the template header title 
        },
       
        methods: {
            changeTitle(){
               $('#titulo_header_content').html('Incomes')               
            },   
            // datatable style  
            table(idtabla){
                this.$nextTick(()=>{  
            ...
            async details(data){
                try{  
                    ...
                    $('#User').html('')
                    $('#Date').html('')
                    let id=data.idhead_income                 
                    this.isLoading = true;
                    await IncomeService.deatilIncome(id).then(res=>{
                        this.isLoading = false;
                        if(res.data.error==true){
                            messageNotificacion("An error occurred while trying to get the category data","error");
                            return
                        }
                        $('#head_income').hide()
                        $('#detail_income').show()
                        this.detail_incomes=res.data
                        $(this.$refs.table_det).DataTable().destroy();
                        this.table('idtable_det')
                        $('#idHeadIncome').html(data.idhead_income)
                    ...
                    if(e.response.status===401){
                        ...
                        messageNotificacion("An error occurred, try again later","error")
                    } 
                }      
